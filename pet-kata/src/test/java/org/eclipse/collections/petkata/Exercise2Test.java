...
public class Exercise2Test extends PetDomainForKata
{
    ...
    {
        Predicate<Person> predicate = x -> x.getPetTypes().contains(PetType.CAT); //replace null with a Predicate lambda which checks for PetType.CAT
    ...
    {
        MutableList<Person> petPeople = this.people.select(p -> p.isPetPerson()); // replace with only the pet owners
